...
import { useNavigate, NavLink } from 'react-router-dom';
import { Search, ShoppingCart } from 'lucide-react';
// استدعاء الـ Context بتاع صاحبك عشان الهيدر يقرا منه
import { useCart } from '../CartContext'; 

export default function Header() {
  const navigate = useNavigate();
  
  // سحبنا المنتجات من الـ Context
  const { items } = useCart();

  // حساب عدد المنتجات في السلة عشان العداد اللي فوق
  // حطينا || [] عشان لو items بـ undefined ميضربش إيرور
    ...
    <nav className="fixed top-0 left-0 w-full z-50 bg-[#06101e]/80 backdrop-blur-md border-b border-gray-800/50 text-white select-none">
      <div className="max-w-[1400px] mx-auto flex items-center justify-between px-8 h-16">
        ...
        <div className="hidden xl:flex items-center gap-7 text-[14px] font-medium">
          ...
          <NavLink to="/datasheets" className={({ isActive }) => isActive ? "relative text-blue-400 font-semibold drop-shadow-[0_0_8px_rgba(96,165,250,0.8)] pb-1 after:absolute after:bottom-0 after:left-0 after:w-full after:h-[2px] after:bg-blue-500 after:shadow-[0_0_10px_rgba(96,165,250,1)]" : "relative text-gray-400 hover:text-white transition-colors pb-1"}>Datasheet Library</NavLink>
