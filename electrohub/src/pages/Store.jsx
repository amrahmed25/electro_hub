    ...
    if (!API_BASE_URL) {
      ...
      fetch(`${API_BASE_URL}/components`)
        ...
        .catch(err => {
          ...
          setError("Failed to load components from Server. Try clearing the API Link.");
          setIsLoading(false);
        });
    }
  }, []);
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#070b14] flex flex-col items-center justify-center text-blue-500">
        ...
        <h2 className="text-xl font-bold text-white tracking-widest animate-pulse">Loading Components...</h2>
      </div>
    );
  }
                ...
                {filteredComponents.map((item, index) => (
                  ...
                  >
                    ...
                    <div className="bg-white rounded-xl h-32 mb-5 p-2 flex items-center justify-center overflow-hidden relative shadow-inner">
                       ...
                       <span className="absolute bottom-1 right-1 bg-black/70 backdrop-blur-sm text-[9px] font-mono text-gray-300 px-1.5 py-0.5 rounded border border-gray-700">{item.package || 'N/A'}</span>
                    </div>
